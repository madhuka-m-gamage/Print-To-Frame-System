...
- Before planning or implementing any change, read `docs/04_workflows/TESTING.md` (its "Planning a change" checklist, coverage map and characterisation register), (its coverage map and register are refreshed on request, tracked in `PLAN.md`, not on every change).
