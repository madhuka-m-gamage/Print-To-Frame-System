...
Full map: [../auth.md](../auth.md). Cross-module chains: [CROSS_MODULE_TRIGGERS.md](../../01_architecture/CROSS_MODULE_TRIGGERS.md). There are no Cloud Functions; all automation is client code (`src/App.jsx`, components) or `api/*.js`.
...
- Google scopes requested: identity only. Drive and Contacts code calls the APIs with that token; `CLAUDE.md` claims otherwise.
