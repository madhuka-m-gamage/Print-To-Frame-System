...
- **Target UI Components**: `src/features/admin/AdminPanel.jsx` (System Overview, storage telemetry, and audit trail), `src/features/admin/AgentDatabase.jsx` (User Management, review queue, member workspace, and account lifecycle), `src/features/admin/PermissionsManager.jsx` (Dynamic role-by-module matrix editor).
...
- **Services & Backend Functions**: `src/features/admin/adminUsers.js` (client wrapper for `/api/admin-user`), `src/services/auditLog.js` (`logActivity`), `api/admin-user.js` (Firebase Admin SDK account provisioning, password reset, and deletion), `src/services/mailer.js` (`sendTemplatedEmail`).
