...
- `src/components/crm/QuotationBuilder.jsx`: embedded in the lead modal; see [cost-calculator-quotation.md](cost-calculator-quotation.md).
...
- A **Deal is a `leads` document** with `isDeal: true`; see [deals.md](deals.md).
...
- **Conversion** (lead in stage "Received" is moved forward or Convert is clicked): creates a Deal, a customer record and a Pending fabrication project. See [deals.md](deals.md).
...
- The `partner_payouts` write path was not traced in this pass (see [partners.md](partners.md)).
- Whether `emitNotification` (`src/utils/events.js`) persists notifications is covered in [notifications.md](notifications.md).
