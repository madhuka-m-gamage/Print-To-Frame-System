...
3. **Ghost / Undefined Message Body in 95%+ of System Notifications**:
   ...
   - In `NotificationsView.jsx:L166`, `<p>{item.message}</p>` renders empty nodes with blank spacing.
4. **Trigger 6c Architectural Disconnect (Ephemeral Self-Targeted Commission Alert)**:
   - In `src/App.jsx:L537-L546`, when both Advance and Final invoices are marked Paid, a commission notification (`type: 'commission'`) is emitted via `emitNotification`.
...
5. **Self-Message Ingestion & Broken "Reply Chat" Loop in `NotificationsView`**:
   - In `NotificationsView.jsx:L31-L47`, direct messages are pulled from `messages.slice(-30)`. The hook does not exclude messages where `fromId === currentUser.identifier`.
...
- `L148-L152` uses `<TwoToneIcon type={item.type || (isOrder ? 'order' : 'system')} size="md" />`.
- `L157-L164` renders the type pill:
  ```jsx
...
- When clicking the header bell (`L961`), sidebar navigation (`L1038`, `L1065`), or mobile menu (`L1444`):
...
- In `handleSignOut` (`src/App.jsx:L874-L879`):
  ```javascript
...
1. **Global Lack of Targeting**:
   ...
   - In `src/context/PermissionsContext.jsx`, every role has `notifications: full()`.
