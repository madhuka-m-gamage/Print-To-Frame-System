...
- `src/features/leads/Leads.jsx`, `LeadCardDetails.jsx` (shared with Deals), `QuotationBuilder.jsx`
- `src/services/gemini.js`, `pricingEngine.js`; `src/features/leads/audioProcessing.js`; `api/generate.js`
...
- Lead and Deal are one lineage: a Deal links back through `originalLeadId` (and the lead forward through `convertedDealId`). Invoices key on `leadId` and may carry either id; receipts key on `invoiceId`. Match through `getLineageIds` / `invoicesForLineage` (`src/features/leads/leadLineage.js`), never a single-id compare.
