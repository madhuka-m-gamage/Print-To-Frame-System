...
| `api/send-email.js` | signed in and approved, **any role** | send an email from the company mailbox to any address, either from a template or with a free subject and body |

## Findings

1. **Any approved account can send email as the company.** `api/send-email.js` has no role check and no limit on recipient or content, so a Partner, Customer or Business Client account can send arbitrary messages from the company mailbox. This is the closest thing to a hidden higher permission. Fix: limit it to staff roles and to the templates, and check the recipient.
