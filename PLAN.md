...
- [ ] Security sweep findings (`docs/03_security/AUTHORIZATION_MAP.md`, 2026-09-21): (a) `api/send-email.js` lets any approved account, including Partner and Customer, send arbitrary email from the company mailbox; limit it to staff roles and templates and check recipients; (b) `api/generate.js` is open to any approved account; (c) the dev proxy in `vite.config.js` skips authentication for creating and resetting Auth users and `npm run dev` binds to `0.0.0.0`; bind to localhost and require the production token checks; (d) console-only checks the repo cannot show: Firebase project IAM members, the role of the service account stored in Vercel, who can edit Vercel variables.
