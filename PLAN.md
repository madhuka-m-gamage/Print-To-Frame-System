...
- [ ] Fabrication F-4: a manually created shop-floor job with a value only ever gets a 25% Final invoice, never the 75% Advance. Owner decision needed: offer an Advance invoice at creation, or treat these as non-billable internal jobs.
- [ ] Fabrication F-6: blueprints are stored as Base64 inside the project document and can pass Firestore's 1MB limit. Move them to Firebase Storage (`blueprints/<jobNo>/<file>`) and store only URLs; needs Storage rules, so it is live-affecting. Until then, cap upload size client-side.
- [ ] Fabrication F-3 (rest): leads and quotations have no frame width, height, depth or profile fields, so a converted job cannot inherit real dimensions. Add capture on the lead or quote, then copy on conversion and run `calculateCutList` there.
